function data = volt_practice_disp(header)
...
data.actualOnsets = nan(par.practice.disp.nStim,1);
data.RT = nan(par.practice.disp.nStim,1);
data.resp = nan(par.practice.disp.nStim,1);
...
for t = 1:par.practice.disp.nStim
    ...
    % calculate fixation color flip onset (with object onscreen)
    objfliptime = stimtime + data.dispStim(t,2)/1000; % trial onset + time to color flip 
    
    % flip fixation to screen at correct onset
    Screen(par.window,'Flip',objfliptime);
...
disp('---------------');
disp(' ');
% disp(sprintf('Please continue by calling %s_disp(header,phase,runNr)',header.exp))
% disp('---------------');
% disp(' ');
